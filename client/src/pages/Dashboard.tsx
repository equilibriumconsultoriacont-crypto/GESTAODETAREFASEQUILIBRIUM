...
function StatCard({ label, value, icon: Icon, color, bg }: { label: string; value: number; icon: React.ElementType; color: string; bg: string }) {
  return (
    <div className="rounded-xl p-4 flex items-center gap-3 border" style={{ background: "#111", borderColor: "#1e4f5c" }}>
...
export default function Dashboard() {
  const now = new Date();
  const { data: stats, isLoading: statsLoading } = trpc.tasks.dashboard.useQuery(undefined, { retry: 2 });
  const { data: dueSoon } = trpc.tasks.dueSoon.useQuery({ days: 3 }, { retry: 2 });
  const { data: clients = [] } = trpc.clients.list.useQuery({ includeInactive: true });
  const clientMap = new Map(clients.map((c) => [c.id, c.name]));
  const { data: allTasks, isLoading: tasksLoading } = trpc.tasks.list.useQuery({}, { retry: 2 });
  ...
  const handleGenerateMonth = async () => {
    try {
      ...
      utils.tasks.dashboard.invalidate();
      utils.tasks.list.invalidate();
    } catch (e: any) { toast.error(e?.message ?? "Erro ao gerar tarefas"); }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: "#a1a1aa" }}>Visão geral — {now.toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <button onClick={handleGenerateMonth} disabled={generateMonthly.isPending}
            ...
            <button onClick={handleMarkOverdue} disabled={markOverdue.isPending}
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-opacity hover:opacity-80"
              style={{ background: "rgba(36,100,108,0.2)", color: "#9fd4dc", border: "1px solid rgba(36,100,108,0.3)" }}>
        ...
        {statsLoading ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            {[...Array(6)].map((_, i) => <div key={i} className="h-20 rounded-xl animate-pulse" style={{ background: "#1a1a1a" }} />)}
          </div>
        ) : (
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            ...
            <div className="space-y-1">
              {dueSoon.map((task) => (
                <Link key={task.id} href={`/tarefas/${task.id}`}>
                  <div className="flex items-center justify-between py-2 px-3 rounded-lg cursor-pointer hover:bg-white/5">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/tarefas"><span className="text-xs cursor-pointer hover:underline" style={{ color: "#9fd4dc" }}>Ver todas →</span></Link>
          </div>
          {tasksLoading ? (
            <div className="space-y-2">{[...Array(5)].map((_, i) => <div key={i} className="h-12 rounded-lg animate-pulse" style={{ background: "#1a1a1a" }} />)}</div>
            ...
            <div className="text-center py-12 rounded-xl border" style={{ borderColor: "#1e4f5c", background: "#111" }}>
              ...
              <p className="text-sm mb-2" style={{ color: "#a1a1aa" }}>Nenhuma tarefa ainda</p>
              <button onClick={handleGenerateMonth} disabled={generateMonthly.isPending}
                className="text-xs px-3 py-1.5 rounded-lg" style={{ background: "#24646c", color: "#fff" }}>
                Gerar tarefas deste mês
              </button>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: "1px solid #1e4f5c" }}>
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium hidden sm:table-cell" style={{ color: "#a1a1aa" }}>Cliente</th>
                ...
                <tbody>
                  {recentTasks.map((task, idx) => (
                    <tr key={task.id} style={{ borderBottom: idx < recentTasks.length - 1 ? "1px solid rgba(30,79,92,0.4)" : "none" }}>
                      <td className="px-4 py-3">
                        <Link href={`/tarefas/${task.id}`}>
                          <div className="flex items-center gap-2 cursor-pointer">
                            <TaskTypeBadge type={task.taskType} />
                            <span className="font-medium hover:underline" style={{ color: "#e5e5e5" }}>{task.title}</span>
                          </div>
                        </Link>
                      </td>
